...
const MyDonationRequests = () => {
  ...
  // Fetch donation requests on load
  useEffect(() => {
    if (!user?.email) {
      setLoading(false);
      return;
  ...
  const deleteDonationRequest = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        // Remove the deleted request from UI
      ...
      <table className="min-w-full table-auto border-collapse">
        ...
        <tbody>
          {currentRequests.length > 0 ? (
            ...
            <tr>
              <td colSpan="5" className="bg-gray-200 dark:bg-gray-500 dark:text-white px-4 py-2 text-center">
                No requests found
              </td>
            </tr>
          )}
